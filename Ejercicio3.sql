--HEMOS AÑADIDO ESTE APARTADO PORQUE NOS DABA ERROR EN EJECUTAR LO MUCHAS VECES.
--Primer boramos las tablas de asosiciones de entidades simples
...
-- Aqui Borraramos tipos (de hijos a padres)
...
-- 3. Definición de Tipos (usamos NOT FINAL para permitir herencia)
...
-- Tipos para Asociaciones (Usando REF)
...
-- 4. Creación de Tablas de Objetos
CREATE TABLE taula_clients OF Client (nif PRIMARY KEY);
CREATE TABLE taula_cursos OF Curs (idCurs PRIMARY KEY);
CREATE TABLE taula_empleats OF Empleat (dni PRIMARY KEY);
CREATE TABLE taula_moduls OF Modul (idModul PRIMARY KEY);
...
-- 5. Insertar datos (Usamos constructores para subtipos)
...
-- 6. Implementación de los Bodies (Importante usar SELF)
CREATE OR REPLACE TYPE BODY Curs AS 
    ...
    BEGIN
        ...
        FROM taula_cursos c 
        WHERE c.idCurs = SELF.idCurs AND VALUE(c) IS OF (CursActiu);
...
-- 7. Consultas de Verificación (CON ALIAS)
-- Ejemplo: Ver si los cursos están activos
SELECT c.nom, c.actiu() AS es_activo FROM taula_cursos c;

-- Ejemplo: Ver módulo actual (usando TREAT para acceder al método del subtipo)
SELECT c.nom, TREAT(VALUE(c) AS CursActiu).modulActual() AS modulo_actual
FROM taula_cursos c
...
-- IMPLEMENTAR LOS METODOS DE LAS CLASES 
...
SELECT c.nom,nif, c.numCursos() AS total
