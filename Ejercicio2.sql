...
-- 1. Empleat (Empleado): comprobar antiguitat() (Años trabajados)
SELECT e.nom, e.data_contracte, e.antiguitat() AS anos_trabajados 
FROM Taula_Empleat e;

-- 2. Alumne (Alumno): comprobar edat() (Edad actual)
SELECT a.nom, a.data_naixement, a.edat() AS edad_actual 
FROM Taula_Alumne a;

-- 3. Investigador: comprobar nivell_recerca() (Nivel de investigación)
SELECT i.nom, i.num_publicacions, i.nivell_recerca() AS nivel_investigacion 
FROM Taula_Investigador i;

-- 4. Administratiu (Administrativo): comprobar sou_anual() (Salario anual = sueldo * 14)
SELECT ad.nom, ad.sou, ad.sou_anual() AS salario_total 
FROM Taula_Administratiu ad;

-- 5. AlumneGrau (Alumno de Grado): comprobar anys_restants() (Años de estudio restantes)
SELECT ag.nom, ag.durada, ag.any_1a_matricula, ag.anys_restants() AS anos_restantes 
FROM Taula_AlumneGrau ag;
